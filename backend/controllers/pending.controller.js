...
async function issueCategory(req, res) {
    ...
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json(new ApiErrorResponse(404, {}, "Item not found"));
        }
...
async function issuePermanentProduct(req, res) {
    const { id, issued_to, issue_date, box_no, uid, source, current_box } = req.body;
    ...
    try {
        ...
        const quantity = boxes.map((box) => parseInt(box.qn)).reduce((a, b) => a + b, 0);
        const query = `
            UPDATE pending
        ...
        const obsHeld = resultBox.map((box) => parseInt(box.qn)).reduce((a, b) => a + b, 0);

        const [result] = await pool.query(query, [issued_to, quantity, box_no, issue_date, id]);
        await pool.query(`UPDATE ${source} SET box_no = ?, obs_held = ? WHERE uid = ?`, [
...
async function servayProduct(req, res) {
    ...
    try {
        ...
        } else if (parseInt(issued_quantity) < parseInt(quantity) + servayedQuantity) {
            return res.status(400).json(new ApiErrorResponse(400, {}, "Quantity exceeded"));
        } else if (parseInt(issued_quantity) == parseInt(quantity) + servayedQuantity) {
            loan_status = "complete";
        }
        if (loan_status == "complete") {
            await pool.query(`UPDATE pending SET loan_status = ? WHERE id = ?`, [loan_status, id]);
        }
        ...
        res.status(200).json(new ApiResponse(200, {}, "Item surveyed successfully"));
    } catch (error) {
...
async function demandProduct(req, res) {
    ...
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json(new ApiErrorResponse(404, {}, "Product not found"));
        }
        res.status(200).json(new ApiResponse(200, {}, "Product demanded successfully"));
    } catch (error) {
        console.log("Error while getting spares: ", error);
        res.status(500).json(new ApiErrorResponse(500, {}, "Internal server error"));
    }
...
async function nacProduct(req, res) {
    ...
    try {
        const query = `
            ...
            WHERE id = ?;
        `;
        const [result] = await pool.query(query, [nac_no, nac_date, rate, validity, id]);
        if (result.affectedRows === 0) {
            return res.status(404).json(new ApiErrorResponse(404, {}, "Product not found"));
        }
...
async function addInInventory(req, res) {
    ...
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(400).json(new ApiErrorResponse(400, {}, "item not found"));
        }
        ...
        res.status(200).json(new ApiResponse(200, {}, "Item added to inventory successfully"));
    } catch (error) {
        if (connection) {
            await connection.rollback();
        }
...
async function pendingIssue(req, res) {
    ...
    try {
        ...
        res.status(200).json(new ApiResponse(200, {}, "Product issued successfully"));
    } catch (error) {
        console.log("Error while getting spares: ", error);
        res.status(500).json(new ApiErrorResponse(500, {}, "Internal server error"));
    }
...
async function addQRCodeResults(req, res) {
    ...
    try {
        ...
        await connection.commit();
        res.status(200).json(new ApiResponse(200, {}, "QR codes added successfully"));
    } catch (error) {
