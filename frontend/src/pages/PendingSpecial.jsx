...
const PendingSpecial = () => {
  ...
  // Helper function to calculate modified OBS
    ...
    const t = fetchedData.items.map((row) => ({
      ...
      quantity:
        row.obs_increase_qty !== null && row.obs_increase_qty !== undefined
          ...
          : "--",
      created_at: getTimeDate(row.created_at),
      // Final expected OBS qty - FIXED CALCULATION
      modified_obs: calculateModifiedOBS(row),

      // Optional: Add tooltip or additional info for clarity
      ...
      >
        ...
        >
          ...
          <div className="flex items-start gap-2 mb-3">
            ...
            <span className="text-gray-900 font-semibold ml-1">
              Base: {selectedRow?.obs_authorised || "-"} | Change:{" "}
              {selectedRow?.obs_increase_qty || 0} | Final:{" "}
              {calculateModifiedOBS(selectedRow)}
            </span>
          </div>
          <DialogDescription className="hidden" />
          <div>
